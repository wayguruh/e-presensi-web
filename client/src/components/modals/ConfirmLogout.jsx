    ...
    <>
      ...
      <div className={`fixed flex-col gap-3 p-6 w-[90%] sm:w-1/2 z-20 left-1/2 -translate-x-1/2 top-1/2 -translate-y-1/2 bg-white rounded-xl text-base text-grey ${props.showModal ? 'flex' : 'hidden'}`}>
        <div className='flex justify-between mb-2'>
          <h3 className='text-black'>Konfirmasi Keluar</h3>
        ...
        <p className='text-grey text-sm'>Apakah Anda yakin ingin keluar?</p>
        <div className='flex justify-end gap-3 mt-2'>
          <button onClick={props.handleLogout} className='flex gap-1 items-center px-3 py-2 bg-alert text-white text-sm font-medium rounded-lg hover:bg-alert/80'>Keluar</button>
